...
disp('**************************************************');
disp(' ');
testscan = 0; % set to 1 if you are just doing an initial analysis on a single volume
if ~testscan
    ...
    basefolder='L:\testscan_28SEP2015\'; % folder in which testscan is saved
    cycles = 0;
    frames = 1;
end
...
end_image=264;%last z slice (top)
x1=36; %these are limits of the region of interest
x2=773;
y1=49;
y2=794; 
rlist = 12; % Bead radius in pixels; should be set to a whole number or +.5
...
for j = rlist %radius -- this loop can be used for bidisperse systems
    ...
    % Create sphere indicies list aka "local_sph"
    ...
    local_sph=local_list((R(:,1)>3/5*j),:);
    clear R;
    ...
    for k=0:cycles%cycle -- this loop is only used for cyclic experiments
        for i=1:frames %frames -- frame number within a cycle (cyclic shear) or overall frame number (steady shear)
            ...
            result=analyze_scan_orientation_one_hole(imagefolder,imageprefix,start_image,end_image,x1,x2,y1,y2,radius,sigma0,AR_z,local_sph_IND,h);
