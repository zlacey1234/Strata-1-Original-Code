...
%BEFORE STARTING load pos_lst into memory, with list of times in the last
%column


function [vtracks,ntracks,meanlength,rmslength] = OuelletteTrackerMJH_3Drotation(pos_lst,max_disp,dim,minarea,inputnames,noisy)
%EDIT by Matt Harrington 16 July 2014: make this appropriate for tracking
%two-hole rotations data; long axis coordinates are stored in A, B, and C,
%while notch coordinates are stored in D, E, and F.

%EDIT by Matt Harrington 5 April 2012: unify 2D and 3D tracking functions
%by including dimension ('dim') as an additional input parameter.  If dim
%is set to 2, a trivial Z struct of all 1's and a trivial W struct of all 
%0's are returned in each track.  Plotting is an option for dim = 2, but 
%not dim = 3.

%EDIT by Matt Harrington 2 April 2012: extend tracking method to three
%dimensions, but removes ability to plot tracks.

%EDIT by Kerstin Nordstrom 26 March 2012: takes position list found via ANY method [x y t] and then uses
%the Ouellette predictive tracking.
%This is the 1st edit by Losert Lab. 
...
% read from the file "bground_name"; see BackgroundImage. If minarea==1, 
% PredictiveTracker seeks single-pixel particles by comparing brightness to 
% adjacent pixels (fast and good for small particles); otherwise 
% PredictiveTracker seeks particles having areas larger than "minarea" (in 
% square pixels; this method is better for tracking large particles). Once 
% identified, each particle is tracked using a kinematic prediction, and a 
...
% horizontal coordinates, vertical coordinates, times, horizontal velocities,
% and vertical velocities of each track, respectively. If minarea~=1, 
% "vtracks" is returned with an additonal field, "Theta", giving the 
% orientation of the major axis of the particle with respect to the x-axis, 
% in radians. The total number of tracks is returned as "ntracks"; the mean 
% and root-mean-square track lengths are returned in "meanlength" and 
% "rmslength", respectively. If noisy~=0, the movie is repeated with overlaid 
% velocity quivers and the tracks are plotted. If noisy==2, each movie frame 
% is also saved to disk as an image. Requires ParticleFinder.m; also 
% requires read_uncompressed_avi.m for use with .avi movies. This file can 
...
% -=- Parse inputs -=-----------------------------------------------------
if nargin<1
    error('Usage: [vtracks,ntracks,meanlength,rmslength] = OuelletteTracker(pos_lst,max_disp,dim,[minarea],[inputnames],[noisy])')
end
...
if dim == 3 && noisy
    error('3D data cannot be plotted in this version; set noisy to 0');
end
...
% -=- Find particles in all frames -=-------------------------------------
 ...
 elseif dim ==3
    z=pos_lst(:,3);
     ...
     error('Invalid dimension!--dim must be set to either 2 or 3');
 end
 t=pos_lst(:,end);
 a=pos_lst(:,end-6);
 b=pos_lst(:,end-5);
...
if minarea==1
    tracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[],'A',[],'B',[],'C',[],'D',[],'E',[],'F',[],'T',[]), ...
        nparticles,1);
    for ii = 1:nparticles
        tracks(ii) = struct('len',1,'X',x(ind(ii)),'Y',y(ind(ii)), ...
            'Z',z(ind(ii)),'A',[],'B',[],'C',[],'D',[],'E',[],'F',[],'T',1);
    end
else
    tracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[],'T',[], ...
        'Theta',[]),nparticles,1);
    for ii = 1:nparticles
        tracks(ii) = struct('len',1,'X',x(ind(ii)),'Y',y(ind(ii)),'Z', ...
            z(ind(ii)),'T',1,'Theta',ang(ind(ii)));
    end
end
...
for t = 2:Nf
    ...
    fr1=[x(ind) y(ind) z(ind) a(ind) b(ind) c(ind) d(ind) e(ind) f(ind)];
    if minarea~=1
        ang1=ang(ind);
    end
...
% -=- Match the tracks with kinematic predictions -=----------------------
    ...
    now = zeros(n_active,3);
    prior = zeros(n_active,3);
    for ii = 1:n_active
        tr = tracks(active(ii));
        now(ii,1) = tr.X(end);
        now(ii,2) = tr.Y(end);
    ...
    if nfr1>0
        ...
        for ii = 1:n_active
            ...
            dist_fr1 = (estimate(ii,1)-fr1(:,1)).^2 + ...
                (estimate(ii,2)-fr1(:,2)).^2 + (estimate(ii,3)-fr1(:,3)).^2;
        ...
        for ii = 1:n_active
            if links(ii) ~= 0 
                ...
                tracks(active(ii)).Z(end+1) = fr1(links(ii),3);
                tracks(active(ii)).A(end+1) = fr1(links(ii),4);
                tracks(active(ii)).B(end+1) = fr1(links(ii),5);
                tracks(active(ii)).C(end+1) = fr1(links(ii),6);
                % check for over-rotation between consecutive frames
                if t > 2 && dot([tracks(active(ii)).A(end-1) tracks(active(ii)).B(end-1) tracks(active(ii)).C(end-1)],[tracks(active(ii)).A(end) tracks(active(ii)).B(end) tracks(active(ii)).C(end)]) < 0
                    tracks(active(ii)).A(end) = -tracks(active(ii)).A(end);
                    tracks(active(ii)).B(end) = -tracks(active(ii)).B(end);
                    tracks(active(ii)).C(end) = -tracks(active(ii)).C(end);
                end
                tracks(active(ii)).D(end+1) = fr1(links(ii),7);
                tracks(active(ii)).E(end+1) = fr1(links(ii),8);
                tracks(active(ii)).F(end+1) = fr1(links(ii),9);
                % Method for computing notch orientation should prevent
                % over-rotation
                %if t > 2 && dot([tracks(active(ii)).D(end-1) tracks(active(ii)).E(end-1) tracks(active(ii)).F(end-1)],[tracks(active(ii)).D(end) tracks(active(ii)).E(end) tracks(active(ii)).F(end)]) < 0
                %    tracks(active(ii)).D(end) = -tracks(active(ii)).D(end);
                %    tracks(active(ii)).E(end) = -tracks(active(ii)).E(end);
                %    tracks(active(ii)).F(end) = -tracks(active(ii)).F(end);
                %end
        ...
        if minarea==1
            newtracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[],...
                'A',[],'B',[],'C',[],'D',[],'E',[],'F',[], ...
                'T',[]),numel(unmatched),1);
            for ii = 1:numel(unmatched)
                newtracks(ii) = struct('len',1,'X',fr1(unmatched(ii),1),...
                    'Y',fr1(unmatched(ii),2),'Z',fr1(unmatched(ii),3),...
                    'A',fr1(unmatched(ii),4),'B',fr1(unmatched(ii),5),...
                    'C',fr1(unmatched(ii),6),'D',fr1(unmatched(ii),7),...
                    'E',fr1(unmatched(ii),8),'F',fr1(unmatched(ii),9),'T',t);
            end
        else
            newtracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[], ...
                'T',[],'Theta',[]),numel(unmatched),1);
            for ii = 1:numel(unmatched)
                newtracks(ii) = struct('len',1,'X',fr1(unmatched(ii),1), ...
                    'Y',fr1(unmatched(ii),2),'Z',fr1(unmatched(ii),3), ...
                    'T',t,'Theta',ang1(unmatched(ii)));
            end
        end
    else % if nfr1>0
        active=[];
...
% loop over tracks
if minarea==1
    vtracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[],'A',[],'B',[]...
        ,'C',[],'D',[],'E',[],'F',[],'T',[],'U',[],'V',[],'W',[]),ntracks,1);
else
    vtracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[],'T',[],'U',[], ...
        'V',[],'W',[],'Theta',[]),ntracks,1);
end
for ii = 1:ntracks
    u = -conv(tracks(ii).X,vkernel,'valid');
    v = -conv(tracks(ii).Y,vkernel,'valid');
    w = -conv(tracks(ii).Z,vkernel,'valid');
    if minarea==1
        vtracks(ii) = struct('len',tracks(ii).len - 2*fitwidth, ...
            'X',tracks(ii).X(fitwidth+1:end-fitwidth), ...
            'Y',tracks(ii).Y(fitwidth+1:end-fitwidth), ...
            'Z',tracks(ii).Z(fitwidth+1:end-fitwidth), ...
            'A',tracks(ii).A(fitwidth+1:end-fitwidth), ...
            'B',tracks(ii).B(fitwidth+1:end-fitwidth), ...
            'C',tracks(ii).C(fitwidth+1:end-fitwidth), ...
            'D',tracks(ii).D(fitwidth+1:end-fitwidth), ...
            'E',tracks(ii).E(fitwidth+1:end-fitwidth), ...
            'F',tracks(ii).F(fitwidth+1:end-fitwidth), ...
...
%         'interpreter','none');
    ...
    elseif numel(names)==1 && ( strcmpi(ext,'.tif') || ...
        ...
        for ii=1:Nf
            if strcmp(fullfile(filepath,names(ii).name),'Background.tif')
    ...
    for ii=1:Nf-3 % all tracks stop 3 frames from end
        np=0;
