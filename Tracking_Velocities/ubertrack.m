...
if notnsqrd
    ...
    %volume = 1; MJH
    minn = min(xyzs(w,1:dim),[],1);
    maxx = max(xyzs(w,1:dim),[],1);
    diff = maxx-minn;
    volume = prod(diff);
%     for d = 0:dim-1
%         minn = min(xyzs(w,d+1));
        ...
        if notnsqrd
            ...
            if ~isempty(deg)
                for j = 0:length(deg)-1
                    cub = cub(cub(:,deg(j+1)) < dimm(deg(j+1)),:); %MJH
                end
            end 
...
%                     end
            ...
            ntrack = length(wh);
            if ntrack == 0 
                disp('There are no valid particles to track idiot!');
                break
            end
            xmat = zeros(ntrack,m);
            count = 0;
