...
function [vtracks,ntracks,meanlength,rmslength] = OuelletteTrackerMJH(pos_lst,max_disp,dim,minarea,inputnames,noisy)
        ...
        if minarea==1
            ...
            for ii = 1:numel(unmatched)
                newtracks(ii) = struct('len',1,'X',fr1(unmatched(ii),1),...
                    'Y',fr1(unmatched(ii),2),'Z',fr1(unmatched(ii),3),'T',t);
            end
        else
            newtracks = repmat(struct('len',[],'X',[],'Y',[],'Z',[], ...
